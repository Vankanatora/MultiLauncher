...
Note: we have to draw ship separately since it has char code 0 and LCDManager::lcd.print() processes it like EOL*/
void updateScreen(){
...
//Sending the buffer to the screen
  for (byte i = 0; i < HEIGHT/2; i++){
    ...
    LCDManager::lcd.print(screenBuffer[i]);
  }
//After all, displaying the ship
  if (!shipDisplayed){
    LCDManager::lcd.setCursor(ship.x(), ship.y()/2);
    LCDManager::lcd.write(byte(SHIP));
  }
}
...
//Displaying a number of level
  ...
  LCDManager::lcd.print("Level ");
  LCDManager::lcd.setCursor(8,0);
  LCDManager::lcd.print(level);
  delay(1000);
  LCDManager::lcd.clear();
}
...
void SpaceInvaders::Run()
{
 ...
 delay(2000);
 LCDManager::lcd.createChar(SHIP, ship_sprite);
 LCDManager::lcd.createChar(BULLET_UP, bullet_up_sprite);
 LCDManager::lcd.createChar(BULLET_DOWN, bullet_down_sprite);
 LCDManager::lcd.createChar(SHIP_BULLET, ship_bullet_sprite);
 LCDManager::lcd.createChar(ALIEN1, alien1_1_sprite);
 LCDManager::lcd.createChar(ALIEN2, alien1_2_sprite);
 LCDManager::lcd.createChar(ALIEN1BULLET, alien1_1_bullet_sprite);
 LCDManager::lcd.createChar(ALIEN2BULLET, alien1_2_bullet_sprite);
 score = 0;
